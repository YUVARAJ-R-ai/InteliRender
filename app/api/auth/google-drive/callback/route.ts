...
export async function GET(req: NextRequest) {
  ...
  if (error) { returnUrl.searchParams.set('error', 'gdrive_denied'); return NextResponse.redirect(returnUrl); }

  const cookieStore = await cookies();
  const savedState = cookieStore.get('gdrive_oauth_state')?.value;
  if (!state || !savedState || state !== savedState) {
    returnUrl.searchParams.set('error', 'gdrive_state');
    return NextResponse.redirect(returnUrl);
  }
  if (!code) { returnUrl.searchParams.set('error', 'gdrive_no_code'); return NextResponse.redirect(returnUrl); }
  ...
  try {
    ...
    if (!tokens.refresh_token) {
      returnUrl.searchParams.set('error', 'gdrive_no_refresh');
      return NextResponse.redirect(returnUrl);
    }

    await db.insert(mcpConnections).values({
      userId: session.user.id, service: 'google_drive',
    ...
    returnUrl.searchParams.set('connected', 'google_drive');
    const response = NextResponse.redirect(returnUrl);
    response.cookies.delete('gdrive_oauth_state');
    return response;
  } catch (err) {
    console.error('Google Drive OAuth error:', err);
    returnUrl.searchParams.set('error', 'gdrive_failed');
    return NextResponse.redirect(returnUrl);
  }
}
